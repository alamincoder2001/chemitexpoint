				...
				<div class="row">
					<div class="col-md-5 col-md-offset-1">
						...
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="text" class="form-control" v-model="payment.SPayment_previous_due">
							</div>
						</div>
					</div>
				...
				<datatable :columns="columns" :data="payments" :filter-by="filter" style="margin-bottom: 5px;">
					<template scope="{ row }">
						<tr>
							...
							<td>
								<button type="button" class="button edit" @click="window.location = `/supplier_payment_invoice/${row.SPayment_id}`">
				...
				axios.post(url, this.payment).then(res => {
					...
					if(r.success){
						...
						if(invoiceConfirm == true){
							window.open('/supplier_payment_invoice/'+r.paymentId, '_blank');
						}
					}
				})
			},
